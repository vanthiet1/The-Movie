    ...
    return (
        <>
            <div className='w-full pt-5'>
                ...
                <div className='flex items-center p-10 gap-8 w-auto overflow-x-auto relative'>
                    <div 
                    onClick={handleLeftArrow} 
                    className="absolute z-10 left-5 bg-slate-500 hover:bg-slate-700 p-5 rounded-full opacity-70">
                    <IconleftArow />
                    </div>
                    <div 
                    onClick={handleRightArrow} 
                    className="absolute z-10 right-5 bg-slate-500 hover:bg-slate-700 p-5 rounded-full opacity-70"
                    >
                    <IconrightArow />
                    </div>
                    ...
                    {data.length > 0 && data.map((itemMovie,index) => (
                        <div key={itemMovie.id} className='relative'>
                            <div className='relative w-96 h-auto transform hover:scale-110 transition-transform ease-in-out duration-500' style={{transform: `translateX(${-currentIndex * 200}%)`}}>
